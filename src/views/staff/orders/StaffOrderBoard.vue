  ...
  <div class="h-[calc(100vh-64px)] bg-gray-100 dark:bg-gray-900 p-4 overflow-hidden flex flex-col">
    <div class="flex justify-between items-center mb-6 shrink-0">
      <h1 class="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-3">
        ...
        Bếp & Pha Chế
        <span
          class="text-xs font-semibold bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200 px-3 py-1 rounded-full shadow-inner"
        >
      ...
      <button
        @click="fetchOrders"
        class="px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 shadow-sm flex items-center gap-2 transition-colors"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
    ...
    <div class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 overflow-hidden min-h-0">
      <div
        class="flex flex-col bg-gray-200 dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-300 dark:border-gray-700"
      >
        <div
          class="p-3 bg-blue-100 dark:bg-blue-900/30 border-b border-blue-200 dark:border-blue-800 flex justify-between items-center"
        >
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
          <div
            ...
            :key="order.id"
            class="bg-white dark:bg-gray-700 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-600 border-l-4 border-l-blue-500 hover:shadow-md transition-shadow relative"
          >
            <div class="absolute top-4 right-4 flex flex-col items-end gap-1.5">
              <span
                v-if="order.orderType === ORDER_TYPE.AT_COUNTER"
                class="bg-purple-50 border border-purple-200 text-purple-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase flex items-center gap-1"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-3 w-3"
              ...
              <span
                v-else
                class="bg-orange-50 border border-orange-200 text-orange-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase flex items-center gap-1"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-3 w-3"
                ...
                >
                  ...
                  />
                  <path
                    d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H10a1 1 0 001-1V5a1 1 0 00-1-1H3zM14 7h-3v7h.05a2.5 2.5 0 004.9 0H17a1 1 0 001-1v-2.828a1 1 0 00-.293-.707l-2.414-2.414A1 1 0 0014.586 7H14z"
                  />
                </svg>
                Giao đi
              </span>
              ...
              <span
                ...
                class="bg-red-50 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded animate-pulse border border-red-200 flex items-center gap-1"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-3 w-3"
            ...
            <div class="mb-3">
              <span class="text-xl font-bold text-gray-800 dark:text-white block"
                >#{{ order.orderCode ? order.orderCode.slice(-4) : '...' }}</span
              >
            ...
            <div
              class="space-y-1.5 mb-4 bg-gray-50 dark:bg-gray-800 p-2.5 rounded-lg border border-gray-100 dark:border-gray-700"
            >
              <div
                v-for="item in order.items"
                :key="item.id"
                class="text-sm text-gray-700 dark:text-gray-200 flex justify-between items-start"
              >
      ...
      >
        <div
          class="p-3 bg-yellow-100 dark:bg-yellow-900/30 border-b border-yellow-200 dark:border-yellow-800 flex justify-between items-center"
        >
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
          <div
            ...
            :key="order.id"
            class="bg-white dark:bg-gray-700 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-600 border-l-4 border-l-yellow-500 hover:shadow-md transition-shadow"
          >
            <div class="flex justify-between items-start mb-3">
              <span class="text-xl font-bold text-gray-800 dark:text-white"
                >#{{ order.orderCode.slice(-4) }}</span
              >
              <span
                class="text-[10px] font-bold bg-gray-100 text-gray-700 dark:bg-gray-600 dark:text-gray-200 px-2 py-1 rounded border border-gray-200 dark:border-gray-500 uppercase flex items-center gap-1"
              >
                <svg
                  v-if="order.orderType === ORDER_TYPE.AT_COUNTER"
                  xmlns="http://www.w3.org/2000/svg"
                ...
                {{
                  order.orderType === ORDER_TYPE.AT_COUNTER
                    ? 'Bàn ' + (order.tableName || '?')
                    : 'Shipper'
                }}
              </span>
            ...
            <div
              class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg border border-gray-100 dark:border-gray-700 mb-4 space-y-3"
            >
              <div
                v-for="item in order.items"
                :key="item.id"
                class="text-sm border-b border-dashed border-gray-200 dark:border-gray-600 last:border-0 pb-2 last:pb-0"
              >
                <div class="font-bold text-gray-800 dark:text-white flex items-start gap-2">
                  ...
                  <span>
                    {{ item.productName }}
                    <span
                      class="text-xs font-normal text-gray-500 bg-white dark:bg-gray-700 px-1.5 py-0.5 rounded border dark:border-gray-600 ml-1"
                      >Bản {{ item.sizeLabel }}</span
                    >
                  </span>
                </div>
                <div class="pl-7 mt-1.5 space-y-1">
                  <div v-if="item.sugarLevel || item.iceLevel" class="flex gap-2">
                    <span
                      v-if="item.sugarLevel"
                      class="text-[11px] font-medium bg-blue-50 text-blue-700 border border-blue-100 px-2 py-0.5 rounded flex items-center gap-1"
                      ><div class="w-1.5 h-1.5 rounded-full bg-blue-500"></div>
                      Đường {{ item.sugarLevel }}</span
                    >
                    <span
                      v-if="item.iceLevel"
                      class="text-[11px] font-medium bg-cyan-50 text-cyan-700 border border-cyan-100 px-2 py-0.5 rounded flex items-center gap-1"
                      ><div class="w-1.5 h-1.5 rounded-full bg-cyan-500"></div>
                      Đá {{ item.iceLevel }}</span
                    >
                  </div>
                  <p
                    v-if="item.note"
                    class="text-xs text-red-600 bg-red-50 p-1.5 rounded border border-red-100 mt-1 flex items-start gap-1"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
      ...
      >
        <div
          class="p-3 bg-green-100 dark:bg-green-900/30 border-b border-green-200 dark:border-green-800 flex justify-between items-center"
        >
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
          <div
            ...
            :key="order.id"
            class="bg-white dark:bg-gray-700 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-600 border-l-4 border-l-green-500 hover:shadow-md transition-shadow"
          >
            <div class="flex justify-between items-center mb-3">
              ...
              >
              <span
                class="text-green-600 font-bold text-sm bg-green-50 px-2.5 py-1 rounded-lg border border-green-100"
                >{{ formatPrice(order.grandTotal) }}</span
              >
            </div>

            <div
              class="text-sm text-gray-700 dark:text-gray-300 mb-4 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3 rounded-lg"
            >
              <p
                class="font-bold mb-2 flex items-center gap-1.5 text-gray-800 dark:text-gray-100 pb-2 border-b border-gray-200 dark:border-gray-600"
              >
                <svg
                  v-if="order.orderType === ORDER_TYPE.AT_COUNTER"
                  xmlns="http://www.w3.org/2000/svg"
                ...
                {{
                  order.orderType === ORDER_TYPE.AT_COUNTER
                    ? 'Khách tại quán'
                    : 'Shipper: ' + (order.shipperName || 'Chưa gán')
                }}
              </p>
            ...
            <div class="space-y-2 mt-2">
              ...
              <button
                ...
                class="w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg font-bold text-sm shadow-sm transition-colors flex items-center justify-center gap-1.5"
              >
                <span
                  v-if="order.orderType === 'delivery' || order.orderType === ORDER_TYPE.DELIVERY"
