...
async function handleAction({ type, item }) {
  if (type === 'view' || type === 'edit' || type === 'row-click') {
    router.push({ name: 'admin.store.detail', params: { id: item.id } })
  }

  if (type === 'delete') {
    const confirmed = await modalStore.confirmDelete(`Bạn có chắc muốn xóa cửa hàng <strong>"${item.name}"</strong>?`)
    if (confirmed) {
      try {
            ...
            <div>
              ...
              <div class="flex items-center gap-2 mt-0.5">
                <button
                  ...
                  class="text-xs font-medium text-green-600 hover:text-green-800 hover:underline flex items-center gap-1 transition-colors"
