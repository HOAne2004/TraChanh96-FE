...
const disableAllSelected = async () => {
  ...
  try {
    ...
    toast.show({
      type: 'success',
      message: `Đã ngừng bán ${selectedProducts.value.length} sản phẩm`,
    })
    selectedAll.value = false
  } catch (err) {
    console.error(err)
...
<template>
  <div class="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm min-h-screen">
    <div class="flex items-center justify-between mb-6">
      <div>
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-1">
          <router-link :to="{ name: 'admin.stores' }" class="underline hover:text-green-600"
            >Quản lý cửa hàng</router-link
          >
          <span>/</span>
    ...
    <div
      v-if="products.length > 0 && selectedProducts.length > 0"
    ...
    >
      ...
      <div class="flex gap-2">
        <button
          ...
          class="px-4 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-150 flex items-center gap-2"
        >
          <svg
            v-if="!bulkProcessing"
            xmlns="http://www.w3.org/2000/svg"
        ...
        <button
          ...
          :disabled="bulkProcessing"
          class="px-4 py-2 bg-gray-600 text-white text-sm rounded-lg hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-150 flex items-center gap-2"
        >
          <svg
            v-if="!bulkProcessing"
            xmlns="http://www.w3.org/2000/svg"
                ...
                <span class="relative">
                  ...
                  >
                    ...
                    <path
                      ...
                      d="M19.5 12h-15"
                    />
                    <rect v-else x="4" y="4" width="16" height="16" rx="1" stroke-width="1.5" />
                  </svg>
                </span>
                <span>
        ...
        <tbody v-if="loading">
          <tr>
            <td colspan="5" class="py-10 text-center text-gray-500">
              <div
                class="inline-block animate-spin rounded-full h-6 w-6 border-b-2 border-green-600 mr-2"
              ></div>
              Đang tải dữ liệu...
            </td>
          </tr>
        ...
        <tbody v-else>
          <tr
            ...
            :class="[
              ...
              p.selected ? 'bg-green-50 hover:bg-green-100' : 'hover:bg-gray-50',
          ...
          >
              ...
              <div class="flex items-center justify-center">
                <div
                  :class="[
                    ...
                    p.selected ? 'bg-green-600 border-green-600' : 'bg-white border-gray-300',
                  ]"
                >
                  <svg
                    v-if="p.selected"
            ...
            <td class="py-3 px-2 text-center">
              <button
                ...
                class="relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-100 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 active:scale-95"
                :class="
                  p.status === 'available'
              ...
              >
                ...
                />
                <span
                  v-if="processingId === p.productId"
                  class="absolute inset-0 flex items-center justify-center"
                >
                  <span
                    class="fast-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"
                  ></span>
                </span>
              </button>
            </td>
