...
const headerColorClass = computed(() => {
  ...
  return props.order.orderType === ORDER_TYPE.AT_COUNTER ? 'bg-green-100' : 'bg-orange-100'
})
...
<template>
  <div
    class="bg-white border rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col h-full"
  >
    <div class="p-3 flex justify-between items-start" :class="headerColorClass">
    ...
    <div class="p-3 flex-1 overflow-y-auto max-h-60 space-y-3">
      <div
        ...
        :key="item.id"
        class="flex justify-between items-start border-b border-dashed border-gray-100 pb-2 last:border-0"
      >
        <div class="flex gap-2">
          ...
          <div class="flex flex-col">
            ...
            <div class="text-xs text-gray-500 mt-1 flex flex-wrap gap-1">
              <span v-if="item.sizeLabel" class="bg-gray-100 px-1 rounded">{{
                item.sizeLabel
              }}</span>
