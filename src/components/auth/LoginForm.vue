...
const handleLogin = async () => {
  ...
  try {
    ...
    else if (auth.user.role === 'STAFF')   router.push('/staff')
    ...
    if (msg.toLowerCase().includes('xác thực') || msg.toLowerCase().includes('kiểm tra email')) {
      ...
      auth.error = null
      // Tự động gửi mã lần đầu
      authService.resendVerification({ email: email.value.trim() }).catch(() => {})
    }
  } finally {
...
// --- Sau khi OtpVerifyForm xác nhận thành công -> đăng nhập luôn ---
...
<template>
  <div class="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
    ...
    <OtpVerifyForm
      ...
      :email="email"
      :external-error="postVerifyError"
