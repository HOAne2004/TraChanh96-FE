...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <div class="flex justify-between items-center mb-4 border-b pb-2">
      ...
      <button v-if="!isEditing" @click="startEdit" class="text-blue-600 text-sm font-medium hover:underline">
        Chỉnh sửa
      </button>
      <div v-else class="flex gap-2">
        <button @click="isEditing = false" class="text-gray-500 text-sm hover:text-gray-700">Hủy</button>
        <button @click="handleSave" class="text-green-600 text-sm font-bold hover:text-green-700">Lưu</button>
      </div>
    </div>

    <div class="flex items-start gap-6">
      <div class="w-24 h-24 rounded-full bg-gray-100 overflow-hidden border border-gray-200 shrink-0">
        <img :src="user.thumbnailUrl || `https://ui-avatars.com/api/?name=${user.username}&size=128`" class="w-full h-full object-cover" />
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
        <div>
          <label class="block text-xs font-medium text-gray-500 uppercase">Họ và tên</label>
          <input v-if="isEditing" v-model="editForm.username" class="mt-1 w-full border-b border-blue-300 focus:border-blue-600 outline-none py-1" />
        ...
        <div>
          ...
          <input v-if="isEditing" v-model="editForm.phone" class="mt-1 w-full border-b border-blue-300 focus:border-blue-600 outline-none py-1" />
