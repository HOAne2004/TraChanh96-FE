...
import sizeService from '@/services/catalog/size.service' // Gọi trực tiếp service cho gọn
      ...
      <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 animate-fade-in">
        ...
        <div class="space-y-4">
          <div>
            ...
            <input
              ...
              type="text"
              placeholder="Ví dụ: Vừa, Lớn..."
              class="w-full border rounded-lg p-2 mt-1 focus:ring-2 focus:ring-green-500 outline-none"
            />
          </div>
          <div>
            <label class="text-xs font-bold text-gray-500 uppercase">Giá mặc định (+)</label>
            <input
              v-model="formData.priceModifier"
              type="number"
              class="w-full border rounded-lg p-2 mt-1 focus:ring-2 focus:ring-green-500 outline-none"
            />
          </div>
        ...
        <div class="flex gap-3 mt-6 justify-end">
          <button
            ...
            class="px-4 py-2 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 text-sm font-bold"
          >
            Hủy
          </button>
          <button
            @click="handleSave"
            class="px-4 py-2 text-white bg-green-600 rounded-lg hover:bg-green-700 text-sm font-bold"
          >
            Lưu
          </button>
        </div>
