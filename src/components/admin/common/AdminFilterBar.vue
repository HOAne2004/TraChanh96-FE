...
const emit = defineEmits(['change', 'export', 'reset'])
...
// 🟢 [SỬA] Tăng thời gian debounce lên 500ms
...
<template>
  <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
    <div class="flex flex-col md:flex-row gap-4 justify-between items-end">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 w-full">
        ...
        <div class="lg:col-span-1">
          ...
          <div class="relative group">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 absolute left-3 top-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
             ...
             <button v-if="filters.search" @click="clearField('search')" class="absolute right-2 top-2 text-gray-400 hover:text-red-500 dark:hover:text-red-400">
        ...
        <div v-if="statusOptions && statusOptions.length > 0">
          ...
          <div class="relative">
            ...
            <button v-if="filters.status !== ''" @click="clearField('status')" class="absolute right-6 top-2.5 text-gray-400 hover:text-red-500 bg-white dark:bg-gray-700">
               <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" /></svg>
            </button>
        ...
        <div class="relative">
          <label class="block text-xs font-medium text-gray-500 mb-1">Từ ngày</label>
           <div class="relative">
            <input v-model="filters.fromDate" type="date" :max="maxFromDate" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 outline-none focus:ring-2 focus:ring-green-500" />
            <button v-if="filters.fromDate" @click="clearField('fromDate')" class="absolute right-8 top-1.5 text-gray-400 hover:text-red-500 bg-white dark:bg-gray-700 px-1">
               <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" /></svg>
            </button>
          </div>
        </div>
        ...
        <div class="flex gap-2">
          <div class="flex-1 relative">
            <label class="block text-xs font-medium text-gray-500 mb-1">Đến ngày</label>
             <div class="relative">
              <input v-model="filters.toDate" type="date" :min="minToDate" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 outline-none focus:ring-2 focus:ring-green-500" />
              <button v-if="filters.toDate" @click="clearField('toDate')" class="absolute right-8 top-1.5 text-gray-400 hover:text-red-500 bg-white dark:bg-gray-700 px-1">
                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" /></svg>
              </button>
            </div>
          </div>
          ...
          <div class="flex items-end">
            <button @click="onReset" class="p-2 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-lg transition-colors dark:bg-gray-700 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600" title="Reset toàn bộ">
      ...
      <button @click="$emit('export')" class="shrink-0 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 text-gray-700 flex items-center gap-2 shadow-sm dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600">
