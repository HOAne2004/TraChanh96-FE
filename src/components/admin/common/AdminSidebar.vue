...
const expandedMenus = ref([]) // Lưu danh sách các menu cha đang mở

// --- CẤU HÌNH MENU ---
const menuItems = [
  {
    title: 'Tổng quan',
    // icon: Chart Bar (Hoặc Home nếu là dashboard chính)
    icon: 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z',
    // Hoặc icon Home cho Dashboard: 'M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M2.25 12v7.5c0 1.036.84 1.875 1.875 1.875h2.25M4.5 19.5h15a2.25 2.25 0 002.25-2.25v-6a2.25 2.25 0 00-2.25-2.25h-14a2.25 2.25 0 00-2.25 2.25v6a2.25 2.25 0 002.25 2.25z'
    path: '/admin',
  ...
  // --- NHÓM 2: SẢN PHẨM ---
  {
    ...
    subItems: [
      ...
      }, // Arrows Right Left (Sort)
    ],
  },

  // --- NHÓM 3: KHO ---
  {
    title: 'Kho hàng (Inventory)',
    // icon: Archive Box (Hoặc Cube)
    icon: 'M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 7.5V3.75m0 0a3.75 3.75 0 017.5 0v3.75m-7.5 0H9m7.5 0H21',
    // Hoặc icon Cube: 'M21 7.5l-2.25-1.313M21 7.5v2.25m0-2.25l-2.25 1.313M3 7.5l2.25-1.313M3 7.5l2.25 1.313M3 7.5v2.25m9 3l2.25-1.313M12 12.75l-2.25-1.313M12 12.75V15m0 6.75l2.25-1.313M12 21.75V19.5m0 2.25l-2.25-1.313m0-16.875L12 2.25l2.25 1.313M21 14.25v2.25l-2.25 1.313m-13.5 0L3 16.5v-2.25',
    ...
    subItems: [
      {
        title: 'Nhân viên',
        path: '/admin/staff',
        icon: 'M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75a2.25 2.25 0 00-2.25-2.25H4.5a2.25 2.25 0 00-2.25 2.25v10.5a2.25 2.25 0 002.25 2.25z',
      }, // Identification (User Group)
    ],
  },
    ...
    subItems: [
      {
        title: 'Cửa hàng',
        path: '/admin/stores',
        icon: 'M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.75m-3.75 0v-7.5M21 10.5a10.5 10.5 0 11-21 0 10.5 10.5 0 0121 0z',
      }, // Building Storefront
      {
        title: 'Cấu hình chung',
        path: '/admin/settings/general',
        icon: 'M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75',
      }, // Adjustments Horizontal (Cog)
      {
        title: 'Sơ đồ Bàn/Phòng',
...
// Kiểm tra item có đang active không
const isActive = (path) => {
  if (path === '/admin') return route.path === '/admin' // Dashboard chính xác
  return route.path.startsWith(path) // Các menu con
}

// Kiểm tra xem menu cha có nên được highlight không (khi con active)
...
const navigateTo = (path) => {
  router.push(path)
  // Trên mobile thì đóng sidebar khi click
  if (window.innerWidth < 1024) {
    isSidebarOpen.value = false
  }
}

// Tự động mở menu cha khi route thay đổi (hoặc F5)
watch(
  () => route.path,
  () => {
...
<template>
  <div>
    ...
    >
      <div class="p-6 border-b border-gray-800 flex flex-col gap-1">
        ...
        <p class="text-xs text-gray-500 uppercase font-semibold pl-10">
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
        <div v-for="item in menuItems" :key="item.name">
          <button
            @click="item.subItems ? toggleSubMenu(item.title) : navigateTo(item.path)"
            :class="[
            ...
            <div class="flex items-center gap-3">
              <svg
                ...
                :class="[
                  ...
                  isParentActive(item) ? 'text-green-500' : 'text-gray-500 group-hover:text-white',
                ]"
              >
          ...
          >
            <div class="pl-4 border-l border-gray-800 ml-6 space-y-1 py-1">
              <button
      ...
      <div class="flex gap-2 p-2 border-t border-gray-800">
        <button
          @click="router.push('/')"
          class="flex items-center gap-3 px-4 py-2 bg-gray-800/10 rounded-xl text-gray-400 hover:bg-gray-800 hover:text-white transition-all group"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
        ...
        >
          ...
          <span class="font-medium text-sm">Về trang nhân viên</span>
        </button>
      </div>
    </aside>
  </div>
...
<style scoped>
/* Custom Scrollbar cho Sidebar đẹp hơn */
