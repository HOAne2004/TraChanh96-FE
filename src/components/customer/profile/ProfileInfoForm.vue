...
const tierStyles = computed(() => {
  ...
  if (name.includes('vàng') || name.includes('gold')) {
    return { bg: 'bg-gradient-to-r from-yellow-400 to-yellow-600', text: 'text-yellow-800', border: 'border-yellow-300' }
  }
  if (name.includes('bạc') || name.includes('silver')) {
    return { bg: 'bg-gradient-to-r from-gray-300 to-gray-400', text: 'text-gray-800', border: 'border-gray-300' }
  }
  if (name.includes('kim cương') || name.includes('diamond')) {
    return { bg: 'bg-gradient-to-r from-cyan-400 to-blue-600', text: 'text-blue-800', border: 'border-blue-300' }
  }
  // Mặc định: Đồng / Member
  return { bg: 'bg-gradient-to-r from-orange-400 to-red-500', text: 'text-orange-800', border: 'border-orange-300' }
})
...
<template>
  <div class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 relative">
    
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 border-b dark:border-gray-700 pb-6 gap-6">
      <div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label for="username" class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Họ và Tên <span class="text-red-500">*</span></label>
          <input
            id="username"
        ...
        <div>
          <label for="phone" class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Số điện thoại</label>
          <div class="relative">
            <input
              id="phone"
              :value="form.phone"
        ...
        <div class="md:col-span-2">
          <label for="email" class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Email đăng nhập</label>
          <input
            id="email"
            :value="form.email"
            type="email"
      ...
      <div class="flex justify-end pt-4 border-t dark:border-gray-700">
        <Button type="submit" :label="isSaving ? 'Đang lưu...' : 'LƯU THAY ĐỔI'" variant="primary" size="lg" :disabled="isSaving" class="w-full md:w-auto px-8"/>
      </div>
    </form>

    <div v-if="isMembershipModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="isMembershipModalOpen = false">
      <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative animate-scale-in">
        
        <button @click="isMembershipModalOpen = false" class="absolute top-4 right-4 z-20 p-1 bg-black/20 text-white rounded-full hover:bg-black/40 transition">
        ...
        <div class="pt-8 pb-6 px-6 relative" :class="tierStyles.bg">
          <div class="text-center text-white relative z-10 mb-6">
            ...
            <p class="text-sm mt-1">Bạn là thành viên hạng <span class="font-bold uppercase">{{ currentLevel.name }}</span></p>
          </div>

          <div class="relative z-10 bg-white/20 backdrop-blur-md border border-white/30 rounded-xl p-5 text-white shadow-inner">
            <div class="flex justify-between items-start mb-4">
              <div>
                <p class="text-[10px] uppercase opacity-70">Mã thẻ (Card Code)</p>
                <p class="font-mono text-lg tracking-widest">{{ membershipInfo.cardCode || 'Đang cập nhật' }}</p>
              </div>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-8 h-8 opacity-50"><path d="M9.375 3a1.875 1.875 0 0 0 0 3.75h1.875v4.5H3.375A1.875 1.875 0 0 1 1.5 9.375v-.75c0-1.036.84-1.875 1.875-1.875h3.193A3.375 3.375 0 0 1 12 2.753a3.375 3.375 0 0 1 5.432 3.997h3.193c1.035 0 1.875.84 1.875 1.875v.75a1.875 1.875 0 0 1-1.875 1.875H12.75v-4.5h1.875a1.875 1.875 0 1 0-1.875-1.875V6.75h-1.5V3h-1.875Z" /><path fill-rule="evenodd" d="M3 15a1.5 1.5 0 0 1 1.5-1.5h15a1.5 1.5 0 0 1 1.5 1.5v4.5a1.5 1.5 0 0 1-1.5 1.5H4.5A1.5 1.5 0 0 1 3 19.5V15Zm6.75 2.25a.75.75 0 0 0 0 1.5h6a.75.75 0 0 0 0-1.5h-6Z" clip-rule="evenodd" /></svg>
            </div>
            <div>
              <p class="text-[10px] uppercase opacity-70">Tổng chi tiêu tích lũy</p>
              <p class="font-bold text-xl">{{ formatPrice?.(membershipInfo.totalSpent) || (membershipInfo.totalSpent + ' đ') }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 bg-gray-50 dark:bg-gray-800">
          ...
          <div class="bg-white dark:bg-gray-700 rounded-xl p-4 shadow-sm mb-6 -mt-10 relative z-20 border border-gray-100 dark:border-gray-600">
            ...
            <div class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2.5 mb-2">
              <div class="bg-green-500 h-2.5 rounded-full transition-all duration-1000 ease-out" :style="`width: ${expPercentage}%`"></div>
            </div>
            ...
            <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 font-medium">
              ...
              <span>1000 Xu</span> </div>
            
            <p class="text-center text-xs text-gray-500 mt-3">
              Tích lũy thêm <span class="font-bold text-green-600">{{ 1000 - (membershipInfo.currentCoins || 0) }} Xu</span> để thăng hạng!
            </p>
          </div>

          <div class="bg-white dark:bg-gray-700 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-600">
            <h4 class="font-bold text-gray-900 dark:text-white mb-4 border-b pb-2 dark:border-gray-600">Thứ hạng hiện tại có gì?</h4>
            
            <ul class="space-y-4">
              <li class="flex gap-3 items-start">
                <div class="mt-0.5 p-1.5 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-md">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-500 mt-0.5">{{ currentLevel.benefits || 'Đang cập nhật quyền lợi' }}</p>
                </div>
              </li>
              
              <li class="flex gap-3 items-start">
                <div class="mt-0.5 p-1.5 bg-orange-100 dark:bg-orange-900/30 text-orange-600 rounded-md">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-500 mt-0.5">Tỷ lệ quy đổi: {{ currentLevel.pointEarningRate || 0 }} (Cứ 100đ = {{ (currentLevel.pointEarningRate || 0) * 100 }} xu)</p>
                </div>
              </li>
            </ul>
          </div>
