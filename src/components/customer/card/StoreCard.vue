...
// Xử lý ảnh
const fullImageUrl = computed(() => resolveImage(props.store.imageUrl, defaultStoreImg))
...
// ⭐ Helper lấy địa chỉ hiển thị
const displayAddress = computed(() => {
    // Nếu API trả về object Address lồng nhau
    if (props.store.address && typeof props.store.address === 'object') {
        return props.store.address.fullAddress ||
               `${props.store.address.addressDetail}, ${props.store.address.province}`
    }
    return 'Đang cập nhật địa chỉ'
})

const link = computed(() => `/aboutUs/${props.store.slug}`)
</script>
    ...
    <router-link :to="link" class="block h-full flex flex-col">
      ...
      <div class="p-5 flex flex-col gap-2">
        ...
        <div class="flex items-start gap-2 text-gray-600 dark:text-gray-300 text-sm">
          ...
          <span class="line-clamp-2">{{ displayAddress }}</span>
        </div>
      </div>
    </router-link>
    <div class="p-5 pt-0">
      <a
        ...
        target="_blank"
        class="mt-3 w-full block text-center py-2 rounded-lg bg-gray-100 hover:bg-green-50 text-green-700 font-semibold text-sm transition-colors border border-transparent hover:border-green-200"
        @click.stop
      >
        Chỉ đường
      </a>
