...
<template>
  <header
    class="h-14 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center px-6 shrink-0 transition-colors duration-300"
  >
    ...
    <div class="flex items-center space-x-4 md:space-x-5">
      ...
      <div
        ...
        :title="isOnline ? 'Hệ thống Online' : 'Mất kết nối'"
      >
        ...
        <span class="text-xs text-gray-500 dark:text-gray-400 hidden sm:inline-block">{{
      ...
      <div class="flex items-center space-x-2">
        <div
          class="w-8 h-8 rounded-full bg-green-100 dark:bg-green-900/50 flex items-center justify-center text-green-600 dark:text-green-300 font-bold text-sm uppercase border border-green-200 dark:border-green-800"
        >
        ...
        <div class="flex flex-col hidden sm:flex">
          ...
          <span class="text-[10px] text-gray-500 dark:text-gray-400">{{
            user?.staff?.position || user?.role || 'Nhân viên'
          }}</span>
        </div>
      </div>
    </div>
