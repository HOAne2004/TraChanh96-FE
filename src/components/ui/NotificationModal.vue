...
<template>
  <div v-if="isVisible" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    >
      <!-- Header -->
      <div
        class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center shrink-0"
      >
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            class="text-sm font-semibold text-blue-600 hover:text-blue-700 hover:underline dark:text-blue-400"
          >
      ...
      <div class="p-6 overflow-y-auto flex-1">
        <div v-if="loading" class="text-center py-10">
          <div
            class="animate-spin h-8 w-8 border-2 border-green-500 border-t-transparent rounded-full mx-auto"
          ></div>
        </div>
        ...
        <div v-else class="space-y-3">
          <div
            ...
            :class="{
              'border-l-4 border-l-blue-500 bg-blue-50/50 dark:bg-blue-900/10': !item.isRead,
            }"
          >
            <div
              class="w-12 h-12 rounded-full bg-white dark:bg-gray-800 shadow-sm flex items-center justify-center text-xl shrink-0"
            >
              {{ getIcon(item.type) }}
            </div>
            ...
            <div class="flex-1 pr-8">
              <div class="flex flex-wrap items-center justify-between mb-1">
                <h4
                  ...
                  :class="{ 'text-blue-600 dark:text-blue-400': !item.isRead }"
                >
                  {{ item.title }}
                </h4>
            ...
            <div
              v-if="!item.isRead"
              class="absolute top-0 right-0 w-3 h-3 bg-blue-500 rounded-bl-lg"
            ></div>
          </div>
        </div>
      </div>
      ...
      <div
        v-if="totalPages > 1"
        class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80 rounded-b-2xl shrink-0 flex justify-between items-center"
      >
        <span class="text-sm text-gray-600 dark:text-gray-400">
          Trang <span class="font-bold text-gray-900 dark:text-white">{{ currentPage }}</span> /
          {{ totalPages }}
        </span>
